...
# Themes also emit icon CSS from PHP -- Unite builds its whole social-icon
# sheet inside inc/socialnav.php -- so scan those too, not just .css files.
css_sources = [theme_dir/'style.css', theme_dir/base/'css'/'custom.css', theme_dir/'rtl.css']
css_sources += [f for f in theme_dir.rglob('*.php')
                if 'tgm' not in str(f) and '/.git/' not in str(f)
                and 'content: "\\f' in f.read_text(errors='ignore')]
for cssf in css_sources:
